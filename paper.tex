...
By combining information from all 9 of the communicate monitors, we will have all the communication traces we need. As an virtualized SOC platform, GEM5 has two types of request, timing and atomic. Timing is what will be used in our system as it simulate delays generated by 
During our experiment, we took only timing request from the raw traces to eliminate initiation traces that are useless for our research.

\subsection{Flow Specification}

In this case study, two system flows are implemented in the SoC model. They include a cache coherent read request from CPU and a cache coherent write request from CPU. In the following , a brief overview for the implemented instances of these flows is given. These flow specifications capture how messages are exchanged for different use cases. In this model, a message is defined with the following format, (Src,Dest,Cmd), where Src and Dest refer to the source and destination components of messages, Cmd refers to the operations that the destination component should perform. 
\begin{figure} 
\centerline{
...
The transaction level model of the simple system implementing the two flow specifications shown in the previous section is implemented by Gem5. We build a simple platform in Gem5 with components described in Fig~\ref{SOC}. By running two simple executables on each CPU simultaneously, we produce a result file including all the communication between each components. The executable assigned to CPU1 is read one file three times for one letter and writing one letter for three times to the same file. CPU2's executable will do the same functionality to the same file, only difference is that CPU2 will write first and then read.
