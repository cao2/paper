...
A central component of post-silicon validation is to
correlate trace from silicon execution with the intended
behavior of the sysem.  As an example, consider a silicon
execution of a smartphone executing a post-silicon use case
where the end-user browses the Web while listening to music
and sending and receiving occasional text messages.  Such a
an execution trace would involve activities from the CPU,
audio controller, display controller, wireless radio
antenna, etc., reflecting the interleaved execution of
...
participating protocols and pinpoint the ``right''
...
SoC designs are composed of a number of design blocks
(called ``intellectual properties'' or ``IPs''), that
coordinate with each other to implement system-level protocols.
